# Scaled Hierarchical Recovery - Report With Exact Code

Notebook: `scaled_hiearchical_recovery.ipynb`
Source view used for copy-paste: `scaled_hiearchical_recovery_cells_extracted.py`

This version pastes the exact code directly in the explanation.

## 1) Imports, reproducibility, and configuration

### Exact code: imports and reproducibility
```python
# ===== CELL 6 =====
import numpy as np
import random
...
## 2) Core helpers and codebook generation

### Exact code: helper utilities
```python
# ===== CELL 10 =====
...
## 3) Channel dataset pipeline (feature + labels)

### Exact code: ChannelDataset (includes `__getitem__`, `g_opt`, `los` extraction)
```python
...
1. Builds an index over weather/town scenario files.
2. Loads NPZ from SSD/flat/ZIP.
3. Creates CSI feature tensor.
4. Extracts labels:
   - `beam_index`
   - `g_opt`
   - `los`
...
## 4) Sensor alignment and multimodal data loading

### Exact code: SensorIndex + sensor frame loader + Multimodal dataset
```python
# ===== CELL 12 =====
...
What this section does:
1. Aligns each channel sample to sensor ZIP/frame.
2. Loads and normalizes LiDAR.
3. Loads IMU from YAML.
4. Returns `(csi, lidar, imu, labels)` where labels are passed through from `ChannelDataset`.

## 5) Trajectory grouping, client creation, and cluster assignment

### Exact code: DatasetSplitter + build_clients + build_clusters
```python
...
What this section does:
1. Creates trajectory IDs by weather/town/location/cav.
2. Splits each trajectory into train/test.
3. Makes one FL client per trajectory.
4. Assigns clients to hierarchical clusters.

## 6) Dataset instantiation and global `beam_change` precompute

### Exact code: dataset build + precompute function (first version)
```python
...
What this section does:
1. Instantiates channel/multimodal datasets.
2. Creates FL clients.
3. Computes `beam_changes_global` once, using trajectory continuity.

## 7) Model architectures

### Exact code: helper gain function + channel/multimodal models
```python
# ── Model Architecture ────────────────────────────────────────────────────────
...
What this section does:
1. Defines channel encoder, LiDAR encoder, IMU encoder, and cross-modal attention.
2. Defines multi-task heads for:
   - `beam`
   - `gopt`
   - `los`
   - `beam_change`

## 8) Channel-only Flower client

### Exact code: BeamFlowerClient (exact label loading + loss usage)
```python
# ===== CELL 21 =====
...
What this section does:
1. Loads all four targets.
2. Applies label smoothing for beam CE.
3. Uses MSE for `g_opt` and weighted BCE for `los` and `beam_change`.
4. Reports auxiliary metrics.

## 9) Hierarchical strategy, attack injection, and recovery logic

### Exact code: HierarchicalFedAvg + AttackingHierarchicalFedAvg
```python
# ===== CELL 22 =====
...
What this section does:
1. Tracks client health signals (`mean_g_opt`, `mean_los`, `mean_beam_change`).
2. Detects suspect CH via 2-of-3 signals.
3. Expels/re-elects/isolate/re-enters clusters.
4. Injects stale weights attack for the targeted CH.

## 10) Multimodal Flower client

### Exact code: MultimodalBeamFlowerClient
```python
# ===== CELL 24 =====
...
What this section does:
1. Same four-label training logic as channel-only.
2. Consumes CSI + LiDAR + IMU tensors.

## 11) Federated training execution (channel-only + multimodal)

### Exact code: channel-only FL run
```python
# ===== CELL 25 =====
...
### Exact code: multimodal FL run and preload optimization
```python
# ===== CELL 26 =====
...
## 12) Visualization and summary output

### Exact code: comprehensive visualization
```python
# ===== CELL 27 =====
...
### Exact code: additional plotting and diagnostics
```python
# ===== CELL 28 =====
...
## 13) Flat-aware override for `beam_change`

### Exact code: override precompute function (later cell)
```python
# ===== CELL 30 =====
...
## 14) Non-beam labels summary (with exact extraction points in pasted code)

1. `g_opt`:
   - Computed in `compute_beam_index` from `gain_per_tx.max()`.
   - Returned in labels dict as `"g_opt"`.
2. `los`:
   - Read from NPZ key `"los"` if available.
   - Else computed from dominant path power ratio threshold `> 0.85`.
3. `beam_change`:
   - Computed globally in `precompute_beam_changes`.
   - Determined from frame-to-frame beam transitions within each trajectory.
